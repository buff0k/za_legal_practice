...
def is_fixture_or_migration_import():
	"""Return true when validation is running during fixture/data import or migration.

	During fixture sync, dependent fixture records may not yet exist in the database
	even though they are present in the app's fixtures directory. In that case, we
	avoid blocking the import on dependency-order checks. Normal Desk/API saves
	remain strict.
	"""
	return bool(
...
class LegalForum(Document):
	...
	def validate_default_party_role(self):
		...
		if not role:
			if is_fixture_or_migration_import():
				return
